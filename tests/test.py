...
def timer(exp):
    ...
    return [elapsed_time.seconds + elapsed_time.microseconds / 1000000, v]

def t(func, args):
    if len(args) > 1:
        rv = timer(f"fustal.{func}({', '.join(args)})")
    else:
        rv = timer(f"fustal.{func}({args[0]})")
    if type(rv[1]) is tuple:
        srv1 = ','.join(map(str, list(rv[1])))
    else:
        srv1 = rv[1]
...
t("one_sample_t_test", ["sepal_length", "6"])

for func in ["two_sample_t_test", "pearson_correlation_coefficient", "simple_linear_regression"]: #, fustal.wilcoxon_rank_sum_test]:
    t(func, ["sepal_length", "sepal_width"])
